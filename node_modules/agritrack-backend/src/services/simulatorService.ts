...
import type { Device, SimulatorState } from "../types/index.js";
...
export async function updateSimulatorState(patch: Partial<SimulatorState>) {
...
export async function tickSimulator() {
  ...
  const result = await persistPosition(device as Device, latitude, longitude);
...
export async function forceSimulatorOutsideFence() {
  ...
  const result = await persistPosition(device as Device, -21.7279, -43.3432);
  return result;
}
...
export async function resetSimulatorPosition() {
  ...
  const result = await persistPosition(device as Device, route[0][0], route[0][1]);
  return { ...result, simulator: state };
}

export async function setSimulatorMode(mode: SimulatorState["mode"]) {
  return updateSimulatorState({ mode });
}

export async function setSimulatorInterval(intervalMs: number) {
  return updateSimulatorState({ intervalMs });
}

export async function startSimulatorLoop(broadcast: BroadcastFn) {
